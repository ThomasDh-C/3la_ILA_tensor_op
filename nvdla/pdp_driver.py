...
import argparse
import timeit
from pdp_converter import PDPConverter


class pdp_driver:
    def __init__(self, op_name, inp_shape, out_shape, pool_size, inp_weight_format,
                 out_format, dilation, padding, strides):
        # 4d array - 1 at start because only batch number 1 supported
        self.orig_inp_shape = [1] + inp_shape
        self.orig_out_shape = out_shape
        self.pool_size = pool_size  # 4d array
        self.inp_weight_format = inp_weight_format  # string
        self.out_format = out_format if out_format else inp_weight_format  # string
    ...
    def produce_main_asm(self):
        ...
        # })
        # WRONG ... SIMULATOR WRONG SO CHANGED PARAM_SHIFT.JSON to WRONG.JSON
        # and CMAC_A_D_MISC_CFG has wrong proc precision
        self.ila_asm.append({
            'name': 'CMAC_A_D_MISC_CFG',
            'NVDLA_CMAC_A_PROC_PRECISION': 0
        })
    ...
    def produce_stripe_asm(self, cache_weight_op, n_n_large, n_c_large, n_h, n_w):
        ...
        for n_h_inp in range(n_h, h-kern_h+1+n_h):
            for n_w_inp in range(n_w, w-kern_w+1+n_w):
                # same kernel weights for all kernels in this stripe - 1 feature atom and 16 kernel atoms supported per os
                # 64 channels
                cmac_op = {'name': 'CMAC_COMPUTE_DOT_PRODUCT'}
                ...
                cmac_op['out_pos'] = f'{n_n_large}_{n_h_inp-n_h}_{n_w_inp-n_w}'
...
if __name__ == '__main__':
    ...
    driver = pdp_driver(op_name=args.op_name[:3],
